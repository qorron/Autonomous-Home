...
if ( exists $config->{host}{symcon} ) {
	...
	if ( $res->is_success ) {
		...
		for my $room ( grep {exists $all{$_}{Raumtemperatur}} keys %all) {
			...
			warn "no: $room $switch_name" unless exists $all{Devices}{'Sw 1'}{$switch_name};
			$rooms->{$plain_room}{powered} = $all{$room}{HeatControlSwitch}{CurrentSwitch} 
				// $all{$room}{'HeatControl PI'}{'Current Switch State'} 
				// $all{$room}{'Heizung'}{'Current Switch State'} 
				// $all{Devices}{'Sw 1'}{$switch_name};;
