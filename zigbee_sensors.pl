...
print temper_hum( $all_sensor_data, $config, 'all', 1 );
print temper_hum( $all_sensor_data, $config, 'temp', 2 );
print temper_hum( $all_sensor_data, $config, 'hum', 3 );
...
sub temper_hum {
    my ( $all_data, $config, $all, $order ) = @_;

    my $title = 'Temperature and humidity';
    my $unit  = '°C/%';
    if ( $all eq 'temp' ) {
        $title = 'Temperature';
        $unit  = '°C';
    ...
    my $return = "multigraph zigbee_temper_hum$sub\n";
    $return .= <<CONFIG if ($config);
graph_title $title for Zigbee sensors
graph_args --base 1000 --lower-limit 0 --upper-limit 100
graph_vlabel $unit
graph_category environment
graph_info This graph shows the $desc reported by all sensors.

CONFIG

    for my $module_key ( sort keys $all_data->%* ) {
        if ($config) {
            $return .= <<CONFIG unless $all eq 'hum';
...
CONFIG
            ...
            $return .= <<CONFIG unless $all eq 'temp';
